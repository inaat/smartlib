<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-md" @click.self="closeModal">
    <div class="bg-white rounded-[2rem] p-8 max-w-md w-full shadow-2xl border border-slate-100 animate-fade-in text-left relative overflow-hidden font-outfit">
      ...
      <div class="absolute bottom-0 left-0 -ml-16 -mb-16 w-36 h-36 rounded-full bg-purple-500/5 blur-3xl"></div>

      <!-- Header -->
      <div class="flex items-center justify-between mb-6 relative z-10">
        <div>
          <h3 class="text-xl font-black text-slate-800">Extend Session</h3>
      ...
      <div class="flex flex-col items-center mb-6 relative z-10">
          ...
          <svg class="w-36 h-36 transform -rotate-90">
            ...
            <circle
              ...
              :class="[
                ...
                remainingMinutes < 15 ? 'stroke-rose-500' : 'stroke-blue-600'
              ]"
              stroke-width="8"
              fill="transparent"
              :stroke-dasharray="2 * Math.PI * 60"
              :stroke-dashoffset="((100 - percentageRemaining) / 100) * (2 * Math.PI * 60)"
              stroke-linecap="round"
            />
          </svg>
          <div class="absolute inset-0 flex flex-col items-center justify-center">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Remaining</span>
            <span class="text-xl font-black font-mono mt-1" :class="remainingMinutes < 15 ? 'text-rose-500' : 'text-slate-800'">
              {{ formatRemainingTime }}
            </span>
          </div>
        </div>
        ...
        <div class="w-full bg-slate-50 border border-slate-100/80 rounded-2xl p-4 flex items-center gap-3.5 mb-6">
          ...
          <div class="min-w-0 flex-1">
            <h4 class="text-xs font-bold text-slate-700">Seat {{ booking?.seat?.seat_number }}</h4>
          ...
          <div v-if="libraryClosingTime" class="text-right flex-shrink-0">
            <p class="text-[9px] font-extrabold uppercase tracking-wider text-slate-400">Library Closes</p>
            <p class="text-xs font-bold text-slate-650">{{ formatTimeOnly(libraryClosingTime) }}</p>
          </div>
        </div>
        ...
        <div v-if="isExtensionRestricted" class="w-full p-4 bg-rose-50 border border-rose-100 rounded-2xl mb-6 flex gap-3 text-left">
          ...
          <div>
            <h5 class="text-xs font-bold text-rose-800">Extension Locked</h5>
        ...
        <div v-else-if="maxExtensionMinutes < 10 || isLibraryClosingSoon" class="w-full p-4 bg-amber-50 border border-amber-100 rounded-2xl mb-6 flex gap-3 text-left">
          ...
          <div>
            <h5 class="text-xs font-bold text-amber-800">Closing Soon</h5>
        ...
        <div v-if="isExtensionAllowed" class="w-full space-y-6 text-left">
          <!-- Quick Add Buttons -->
          <div>
            <label class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 block mb-2.5">Quick Add Extension</label>
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Custom Duration</label>
            ...
            <div class="flex items-center gap-4">
              <span class="text-[10px] font-extrabold text-slate-450 font-mono">10m</span>
              <input
                type="range"
                v-model.number="extensionDuration"
                :min="10"
              ...
              />
              <span class="text-[10px] font-extrabold text-slate-450 font-mono">{{ maxExtensionMinutes }}m</span>
            </div>
          </div>

          <!-- Live Preview Card -->
          <div class="bg-indigo-50/40 border border-indigo-100/50 rounded-2xl p-4.5 space-y-3">
            <h5 class="text-[9.5px] font-extrabold text-indigo-500 uppercase tracking-widest">Time Extension Preview</h5>
            <div class="grid grid-cols-3 items-center text-center gap-1">
              <div>
                ...
                <p class="text-xs font-bold text-slate-700 font-mono">{{ formatTimeOnly(currentEndTime) }}</p>
              </div>
              <div class="flex flex-col items-center">
                <ArrowRight class="w-4 h-4 text-indigo-400" />
                <span class="text-[10px] font-black text-indigo-650 mt-0.5 bg-indigo-50 px-2 py-0.5 rounded-md font-mono">
                  {{ formatAddedTime }}
                </span>
              </div>
              <div>
