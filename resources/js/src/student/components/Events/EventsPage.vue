<template>
  <div class="space-y-6 pb-12 font-outfit text-left">
      ...
      >
        ...
        <div class="p-5 flex-1 flex flex-col">
          <div class="flex justify-between items-start mb-3">
             <span class="inline-block text-[9px] font-extrabold text-blue-600 bg-blue-50/50 px-2 py-0.5 rounded uppercase tracking-wider">
          ...
          <h3 class="font-bold text-slate-800 text-base mb-1.5 leading-snug group-hover:text-blue-600 transition-colors line-clamp-1" :title="event.title">{{ event.title }}</h3>
          ...
          <div class="mt-auto pt-4 border-t border-slate-100 flex flex-col space-y-2">
            <div class="flex items-center justify-between text-[10px] text-slate-400 font-extrabold uppercase tracking-wide">
              <span>Attendees</span>
    ...
    <div v-else class="text-center py-20 bg-slate-100 rounded-2xl border border-dashed border-slate-200/80 p-8">
      ...
      <h3 class="text-base font-bold text-slate-800 mb-1">No upcoming events</h3>
    ...
    <div v-if="isPassModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md overflow-y-auto animate-in fade-in duration-300">
      <div class="max-w-md w-full bg-slate-100 rounded-[32px] border border-slate-200 shadow-2xl overflow-hidden relative flex flex-col my-8">
        ...
        <div class="p-5 flex items-center justify-between border-b border-slate-200/80 bg-slate-50">
          <span class="text-xs font-black uppercase tracking-widest text-slate-500">Digital Event Pass</span>
          ...
          <div ref="passRef" class="w-full max-w-sm bg-white rounded-[24px] border border-slate-200/80 shadow-lg overflow-hidden relative flex flex-col font-outfit text-slate-800" style="width: 360px;">
            ...
            <div class="p-5 bg-gradient-to-r from-blue-600 to-blue-700 border-b border-blue-700/20 flex items-center space-x-3 text-white">
              ...
              <div class="min-w-0 flex-1">
                <h4 class="text-sm font-black uppercase tracking-[0.15em] text-white">GATE PASS</h4>
              </div>
            </div>
            ...
            <div class="p-5 space-y-4">
              <div class="space-y-1">
                <span class="inline-block text-[9px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-150 uppercase tracking-widest">
                  {{ selectedPassEvent?.type?.replace('_', ' ') || 'Workshop' }}
                </span>
                <h3 class="text-lg font-extrabold leading-tight text-slate-900 tracking-tight line-clamp-2">
              ...
              <div class="grid grid-cols-2 gap-4 bg-slate-50 border border-slate-100 rounded-xl p-3.5">
                <div>
                  ...
                  <p class="text-[11px] font-extrabold text-slate-800 mt-0.5">{{ selectedPassEvent ? formatDate(selectedPassEvent.date) : '' }}</p>
                </div>
                <div>
                  <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Time</span>
                  <p class="text-[11px] font-extrabold text-slate-800 mt-0.5">{{ selectedPassEvent ? formatTime(selectedPassEvent.start_time) : '' }}</p>
                </div>
                <div>
                  <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Venue</span>
                  <p class="text-[11px] font-extrabold text-slate-800 mt-0.5 truncate">{{ selectedPassEvent?.venue || 'Main Hall' }}</p>
                </div>
                <div>
                  <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Seat</span>
                  <p class="text-[11px] font-extrabold text-blue-600 mt-0.5">General Admission</p>
                </div>
              </div>
            </div>
            ...
            <div class="p-5 flex items-center justify-between space-x-4 bg-slate-50/50">
              ...
              <div class="min-w-0 flex-1 space-y-4">
                <div class="flex items-center space-x-3">
                  ...
                  <div class="min-w-0">
                    <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Attendee</span>
                    <h5 class="text-xs font-black truncate text-slate-900 leading-snug">{{ user?.name }}</h5>
                    <p class="text-[9px] text-blue-600 font-extrabold uppercase mt-0.5">Verified</p>
                  </div>
                </div>

                <div>
                  <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Student CRN</span>
