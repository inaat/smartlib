<template>
  <div class="space-y-6 pb-12 font-outfit">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-2.5">
            <div class="text-left">
              <h3 class="font-extrabold text-slate-800 text-base leading-snug line-clamp-1" :title="reservation.book?.title">{{ reservation.book?.title }}</h3>
          ...
          <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-slate-500">
            ...
            <div class="flex items-center">
              ...
              <span class="font-bold uppercase tracking-wide text-[10px]" :class="getDaysRemainingColor(reservation.due_date)">
        ...
        <div class="flex flex-col gap-2 w-full md:w-auto flex-shrink-0">
          ...
          <div v-else-if="reservation.status === 'pending_return'" class="text-center text-xs text-slate-400 font-extrabold uppercase tracking-wide bg-slate-50 border border-slate-100/50 rounded-xl px-4 py-2">
            Pending Approval
          </div>
        </div>
      </div>
    ...
    <div v-else class="text-center py-20 bg-slate-100 rounded-2xl border border-dashed border-slate-200/80 p-8 font-outfit">
      ...
      <h3 class="text-base font-bold text-slate-800 mb-1">No active reservations</h3>
