...
const StudentSidebar: React.FC<StudentSidebarProps> = ({ isOpen, onClose }) => {
  ...
  const mainNavItems = [
    ...
    { path: '/libraries', label: 'Find Libraries', icon: MapPin },
    { path: '/books', label: 'Browse Books', icon: BookOpen },
    { path: '/events', label: 'Events & Seminars', icon: Calendar },
    { path: '/qr-checkin', label: 'QR Check-In', icon: QrCode },
    { path: '/profile', label: 'My Profile', icon: User },
  ];

  const quickAccessItems = [
    { path: '/profile?tab=subscription', label: 'Subscription', icon: CreditCard },
    { path: '/profile?tab=loyalty', label: 'Loyalty Points', icon: Gift },
    { path: '/profile?tab=analytics', label: 'Study Analytics', icon: BarChart3 },
  ];
