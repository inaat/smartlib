<template>
  <div class="space-y-5 text-slate-700 font-outfit">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-5">
      ...
      >
        <div class="absolute -right-4 -top-4 text-slate-50/10 group-hover:scale-105 transition-transform duration-500 pointer-events-none">
        ...
        <div class="flex justify-between items-start relative z-10">
          <div>
            ...
            <h4 class="text-2xl sm:text-3xl font-extrabold mt-1.5 sm:mt-2 tracking-tight">{{ stats.all }}</h4>
          </div>
          <div :class="['p-2 sm:p-2.5 rounded-xl border', filterStatus === 'all' ? 'bg-white/10 border-white/20 text-white' : 'bg-slate-50 border-slate-100 text-slate-500']">
      ...
      >
        ...
        <div class="flex justify-between items-start relative z-10">
          <div>
            ...
            <h4 class="text-2xl sm:text-3xl font-extrabold mt-1.5 sm:mt-2 tracking-tight">{{ stats.open }}</h4>
          </div>
          <div :class="['p-2 sm:p-2.5 rounded-xl border', filterStatus === 'open' ? 'bg-white/10 border-white/20 text-white' : 'bg-amber-50 border-amber-100/80 text-amber-600']">
            <Clock class="w-4 h-4" />
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div class="flex justify-between items-start relative z-10">
          <div>
            ...
            <h4 class="text-2xl sm:text-3xl font-extrabold mt-1.5 sm:mt-2 tracking-tight">{{ stats.closed }}</h4>
          </div>
          <div :class="['p-2 sm:p-2.5 rounded-xl border', filterStatus === 'closed' ? 'bg-white/10 border-white/20 text-white' : 'bg-emerald-50 border-emerald-100/80 text-emerald-600']">
    ...
    <div class="flex flex-col md:flex-row h-[calc(100vh-14rem)] min-h-[500px] max-h-[750px] bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden font-outfit">
      
      <!-- Left sidebar: Ticket List -->
      <div :class="[
        'w-full md:w-80 lg:w-1/3 border-r border-slate-100 flex flex-col bg-slate-50/20 flex-shrink-0 h-full',
        selectedTicket && mobileView === 'chat' ? 'hidden md:flex' : 'flex'
      ]">
              ...
              :class="[
                ...
                selectedTicket?.id === ticket.id 
                  ? (userRole === 'student' 
                      ...
                      : (userRole === 'admin' ? 'bg-purple-50/40 border-purple-600' : 'bg-emerald-50/40 border-emerald-600'))
                  : 'border-transparent'
              ]"
            >
              ...
              <p class="text-[10px] text-slate-400 line-clamp-1 mb-2 pr-2 font-medium">{{ ticket.messages?.[0]?.message || 'No messages' }}</p>

              <!-- Footer with author & date -->
              <div class="flex justify-between items-center text-[10px] text-slate-450 font-semibold mt-1">
                <span class="flex items-center text-slate-500 truncate mr-2">
                  <User class="w-3.5 h-3.5 mr-1 text-slate-400 flex-shrink-0" />
                  <span class="truncate">{{ ticket.user?.name || 'User' }}</span>
                </span>
                <span class="text-slate-400 flex-shrink-0">{{ formatDate(ticket.created_at) }}</span>
              </div>
      ...
      <div :class="[
        ...
        !selectedTicket || mobileView === 'list' ? 'hidden md:flex' : 'flex'
      ]">
        <template v-if="selectedTicket">
          
          <!-- Conversation Header -->
          <div class="bg-white p-3.5 sm:p-4 border-b border-slate-100 flex justify-between items-center shadow-sm relative z-10 text-left">
            <div class="flex items-center space-x-2.5 sm:space-x-3.5 min-w-0">
              ...
              <div class="w-8 h-8 sm:w-9 sm:h-9 border border-slate-100 rounded-xl bg-slate-50 flex items-center justify-center text-slate-600 flex-shrink-0">
                <User class="w-4 h-4" />
              </div>
              <div class="min-w-0">
                <h3 class="font-bold text-slate-800 text-xs sm:text-sm leading-tight truncate">{{ selectedTicket.subject }}</h3>
                <p class="text-[10px] text-slate-400 mt-0.5 font-semibold flex flex-wrap items-center gap-1.5 leading-none">
                  Ticket #{{ selectedTicket.id }} • From: <span class="font-bold text-slate-600">{{ selectedTicket.user?.name }}</span>
            ...
            <div class="flex items-center space-x-1.5 sm:space-x-2 flex-shrink-0">
              ...
              <span v-if="selectedTicket.status === 'closed'" class="px-2 py-1 rounded-xl bg-slate-100 border border-slate-200 text-slate-500 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
          ...
          <div class="flex-1 overflow-y-auto p-4 sm:p-6 space-y-3.5 bg-slate-50/30" ref="messageContainer">
            <div 
          ...
          <div v-if="selectedTicket.status === 'closed'" class="p-3.5 sm:p-4 bg-slate-100/80 border-t border-slate-200 text-center">
            <p class="text-xs font-bold text-slate-500 flex items-center justify-center gap-1.5">
              <Lock class="w-4 h-4 text-slate-400" />
              This complaint is completely closed and locked.
            </p>
          </div>
          <div v-else class="p-3 sm:p-4 bg-white border-t border-slate-100">
            ...
            <p class="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mt-2 text-center hidden sm:block">Press Enter to send (Shift+Enter for newline)</p>
          </div>
        </template>

        <!-- No Ticket Selected Empty State -->
        <div v-else class="flex-1 flex flex-col items-center justify-center p-8 sm:p-12 text-center h-full">
          <div class="w-14 h-14 sm:w-16 sm:h-16 border border-slate-100 rounded-2xl flex items-center justify-center mb-4 bg-white text-slate-400">
            <LifeBuoy class="w-6 h-6 animate-pulse" />
          </div>
          <h3 class="text-xs sm:text-sm font-bold text-slate-800 mb-1 uppercase tracking-wider">Support Desk</h3>
    ...
    <div v-if="showCreateModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div class="relative bg-white rounded-3xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto border border-slate-100 text-left animate-in duration-200">
        
        <!-- Modal Header -->
        <div class="p-5 sm:p-6 border-b border-slate-100 flex justify-between items-center text-left">
          <div>
            <h2 class="text-base sm:text-lg font-bold text-slate-800">Submit Support Ticket</h2>
          </div>
        ...
        <!-- Modal Form -->
        <form @submit.prevent="handleCreateTicket" class="p-5 sm:p-6 space-y-4 text-left">
          <div>
