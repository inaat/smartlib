    ...
    <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
      <div class="flex flex-col md:flex-row items-center gap-8">
        <div class="relative group">
          <div class="w-32 h-32 rounded-full bg-gradient-to-tr from-indigo-600 to-blue-600 flex items-center justify-center text-white text-4xl font-bold shadow-lg overflow-hidden">
            <img 
        ...
        <div class="flex-1 text-center md:text-left">
          ...
          <div class="flex flex-wrap justify-center md:justify-start gap-4 text-gray-600">
            <div class="flex items-center">
              <Mail class="w-4 h-4 mr-2 text-indigo-500" />
              {{ user?.email }}
            </div>
            <div class="flex items-center" v-if="user?.phone">
              <Phone class="w-4 h-4 mr-2 text-indigo-500" />
              {{ user?.phone }}
            </div>
          </div>
          
          <div class="mt-6 flex flex-wrap justify-center md:justify-start gap-3">
            <span class="px-4 py-1.5 bg-indigo-50 text-indigo-700 rounded-full text-sm font-bold border border-indigo-100">
              SUPER ADMIN
            </span>
          </div>
        </div>
...
import { studentAPI } from '@/shared/services/api'; // Reusing studentAPI for profile updates
import { 
  User as UserIcon, 
  Mail, 
  Phone, 
