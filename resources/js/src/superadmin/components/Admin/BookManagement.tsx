...
const BookManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.type === 'physical') {
        formDataToSend.append('library_id', formData.libraryId);
        ...
        // Add digital access settings
        formDataToSend.append('access_level', formData.digitalAccess.accessLevel);
        formDataToSend.append('subscription_required', formData.digitalAccess.subscriptionRequired ? '1' : '0');
        if (formData.digitalAccess.passphrase) {
          formDataToSend.append('passphrase', formData.digitalAccess.passphrase);
        }
      }
      ...
      // Reload books list (you may need to add this to AppContext)
  ...
  const handleEdit = (book: any) => {
    // Normalize digital access data
    const digitalAccess = book.digitalAccess ? {
      subscriptionRequired: Boolean(book.digitalAccess.subscriptionRequired),
      passphrase: book.digitalAccess.passphrase || '',
      pdfUrl: book.digitalAccess.pdfUrl || '',
      accessLevel: book.digitalAccess.accessLevel || 'free'
    } : {
      subscriptionRequired: false,
      passphrase: '',
      pdfUrl: '',
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {formData.type === 'digital' && (
              <div className="space-y-4 p-4 bg-blue-50 rounded-lg">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Upload PDF Book</label>
                  <div className="flex items-center space-x-4">
                    ...
                    {(pdfFileName || formData.digitalAccess.pdfUrl) && (
                      <div className="flex items-center space-x-2 text-sm text-gray-700">
                        <Upload className="w-4 h-4 text-green-600" />
                        <span className="font-medium">{pdfFileName || 'PDF uploaded'}</span>
                      </div>
                    )}
                  </div>
                  <p className="text-xs text-gray-500 mt-1">Upload the digital book file (PDF format)</p>
                </div>
