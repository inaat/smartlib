<template>
  <div class="space-y-6">
    ...
    <!-- Orders Table -->
    <div v-if="activeTab === 'orders'" class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            <tr v-else-if="orders.length === 0">
              <td colspan="7" class="px-6 py-12 text-center text-gray-500">No orders found matching your criteria.</td>
            </tr>
            <tr v-for="order in orders" :key="order.id" class="hover:bg-gray-50/50 transition-colors">
              <td class="px-6 py-4">
                <span class="text-sm font-medium text-gray-900">#ORD-{{ order.id }}</span>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center space-x-3">
              ...
              <td class="px-6 py-4">
                <span class="text-sm text-gray-600">{{ order.plan?.name || 'N/A' }}</span>
              </td>
              <td class="px-6 py-4">
                <span class="text-sm font-bold text-gray-900">PKR {{ order.amount?.toLocaleString() }}</span>
              </td>
              <td class="px-6 py-4">
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            <tr v-for="sub in subscriptions" :key="sub.id" class="hover:bg-gray-50/50 transition-colors">
              <td class="px-6 py-4">
                <span class="text-sm font-medium text-gray-900">#SUB-{{ sub.id }}</span>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center space-x-3">
                  <div class="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-xs">
              ...
              <td class="px-6 py-4">
                <span class="text-sm text-gray-600">{{ sub.subscription_plan?.name || 'N/A' }}</span>
              </td>
              <td class="px-6 py-4">
                <span class="text-sm text-gray-500">{{ formatDate(sub.started_at) }}</span>
              </td>
              <td class="px-6 py-4">
                <span class="text-sm text-gray-500">{{ formatDate(sub.expires_at) }}</span>
              </td>
              <td class="px-6 py-4">
                <span :class="['px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider', getStatusClass(sub.status)]">
                  {{ sub.status }}
                </span>
              </td>
            </tr>
    ...
    <div v-if="showRejectModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div class="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h3 class="font-bold text-gray-900">Reject Order #ORD-{{ selectedOrder?.id }}</h3>
        ...
        <div class="px-6 py-4 bg-gray-50 flex items-center justify-end space-x-3">
          ...
          <button 
            @click="confirmReject"
            :disabled="rejecting"
            class="px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50 flex items-center space-x-2"
          >
            <span v-if="rejecting" class="animate-spin border-2 border-white/30 border-t-white rounded-full w-4 h-4"></span>
...
const fetchSubscriptions = async (page = 1) => {
  try {
    loading.value = true;
    const response = await superadminAPI.getUserSubscriptions({
      page,
      search: search.value,
      status: statusFilter.value
    });
