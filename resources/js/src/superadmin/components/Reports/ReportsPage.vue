<template>
  <div class="space-y-8 pb-12 font-outfit text-slate-700">
      ...
      <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden relative">
        <div class="p-8 space-y-6">
          ...
          <div class="bg-slate-50 rounded-xl p-3 flex flex-wrap items-center justify-between text-xs font-bold text-slate-500 text-left gap-2 border border-slate-100">
            <div>{{ generatedReport.headerLabel || 'Generated By:' }} <span class="text-slate-800">{{ generatedReport.generatedBy.name }}</span></div>
                ...
                <template v-else>
                  ...
                  >
                    <td v-for="(val, colIdx) in Object.values(row).slice(1)" :key="colIdx" class="py-3 px-4 font-semibold text-slate-655 whitespace-nowrap">
                      <span v-if="colIdx === Object.keys(row).length - 2 && isStatusField(String(val))" v-html="statusBadgeHtml(String(val))"></span>
    ...
    <!-- Scheduled & Automated Reports Section for Super Admin -->
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden text-left space-y-0">
      <!-- Header Row -->
      <div class="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center space-x-3.5">
          <div class="w-11 h-11 rounded-2xl bg-purple-50 flex items-center justify-center text-purple-700 border border-purple-100 flex-shrink-0">
            <CalendarDays class="w-6 h-6" />
          </div>
          <div>
            <h3 class="text-base font-bold text-slate-800 tracking-tight">Automated Library Reports Hub</h3>
            <p class="text-xs text-slate-500 font-medium mt-0.5">View, manage and download daily, weekly & monthly reports scheduled by librarians across libraries.</p>
          </div>
        </div>
      ...
      <!-- Navigation Sub-Tabs Row (Below Header) -->
      <div class="bg-slate-50/70 border-b border-slate-100 px-6 py-2.5 flex items-center space-x-2">
        <button
          @click="activeSubTab = 'scheduled'"
          :class="[
        ...
        >
          ...
          <span>Scheduled Automations ({{ displayScheduledReports.length }})</span>
        </button>
        <button
        ...
        >
          ...
          <span>Generated Reports Archive ({{ displayGeneratedReports.length }})</span>
        </button>
      </div>

      <!-- Scheduled Automations List -->
      <div v-if="activeSubTab === 'scheduled'" class="p-6">
        <div v-if="displayScheduledReports.length === 0" class="text-center py-12 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
          <CalendarDays class="w-10 h-10 text-slate-300 mx-auto mb-3" />
          <p class="text-slate-400 text-xs font-bold uppercase tracking-wider">No automated report schedules match your selection.</p>
        </div>
          ...
          <table class="w-full text-xs">
            <thead>
              <tr class="bg-slate-50/80 text-slate-400 font-bold uppercase tracking-wider border-b border-slate-100">
                <th class="py-3 px-4 font-bold text-left">Library Name</th>
                <th class="py-3 px-4 font-bold text-left">Librarian</th>
                <th class="py-3 px-4 font-bold text-left">Report Type</th>
                <th class="py-3 px-4 font-bold text-left">Frequency</th>
                <th class="py-3 px-4 font-bold text-left">Format</th>
                <th class="py-3 px-4 font-bold text-left">Delivery Time</th>
                <th class="py-3 px-4 font-bold text-left">Status</th>
                <th class="py-3 px-4 font-bold text-right">Action</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 bg-white">
              <tr v-for="s in displayScheduledReports" :key="s.id" class="hover:bg-slate-50/50 font-semibold text-slate-650">
                <td class="py-3.5 px-4 font-bold text-slate-800">
                  <span>{{ s.library_name }}</span>
                </td>
                <td class="py-3.5 px-4">
                  <span class="block text-slate-800 font-bold">{{ s.librarian_name }}</span>
                  <span class="text-[10px] text-slate-400 font-medium">{{ s.recipient_email }}</span>
                </td>
                <td class="py-3.5 px-4 capitalize font-bold text-purple-700">{{ s.report_type }}</td>
                <td class="py-3.5 px-4">
                  <span :class="[
                    'px-2.5 py-1 rounded-lg text-[10px] font-extrabold uppercase border',
                    s.frequency === 'daily' ? 'bg-blue-50 border-blue-100 text-blue-700' :
                    s.frequency === 'weekly' ? 'bg-purple-50 border-purple-100 text-purple-700' :
                    'bg-amber-50 border-amber-100 text-amber-700'
                  ]">
                    {{ s.frequency }} Base
                  </span>
                </td>
                <td class="py-3.5 px-4 uppercase text-[10px] font-bold">
                  <span :class="s.format === 'pdf' ? 'text-red-600 bg-red-50 px-2 py-0.5 rounded border border-red-100' : 'text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100'">
                    {{ s.format }}
                  </span>
                </td>
                <td class="py-3.5 px-4 font-medium text-slate-500">{{ s.send_time || '08:00' }}</td>
                <td class="py-3.5 px-4">
                  <span :class="s.status === 'active' ? 'text-emerald-700 bg-emerald-50 border border-emerald-100 px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase' : 'text-slate-500 bg-slate-100 px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase'">
                    {{ s.status }}
                  </span>
                </td>
                <td class="py-3.5 px-4 text-right">
                  <div class="flex items-center justify-end gap-1.5">
                    ...
                    <button
                      @click="downloadScheduledReport(s)"
      ...
      <div v-if="activeSubTab === 'history'" class="p-6">
        <div v-if="displayGeneratedReports.length === 0" class="text-center py-12 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
          <History class="w-10 h-10 text-slate-300 mx-auto mb-3" />
          <p class="text-slate-400 text-xs font-bold uppercase tracking-wider">No generated report archives match your selection.</p>
        </div>
        <div v-else class="overflow-x-auto rounded-xl border border-slate-100">
          <table class="w-full text-xs">
            <thead>
              <tr class="bg-slate-50/80 text-slate-400 font-bold uppercase tracking-wider border-b border-slate-100">
                <th class="py-3 px-4 font-bold text-left">Library</th>
            ...
            <tbody class="divide-y divide-slate-100 bg-white">
              <tr v-for="g in displayGeneratedReports" :key="g.id" class="hover:bg-slate-50/50 font-semibold text-slate-650">
                <td class="py-3 px-4 font-bold text-slate-800">
                  <span>{{ g.library_name }}</span>
                </td>
                <td class="py-3 px-4 font-bold text-purple-900">{{ g.report_title }}</td>
                <td class="py-3 px-4 capitalize">
                  <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase border bg-slate-50 border-slate-200 text-slate-700">
                    {{ g.frequency }} Base
                  </span>
                </td>
                <td class="py-3 px-4 uppercase text-[10px] font-bold">
                  <span :class="g.format === 'pdf' ? 'text-red-600 bg-red-50 px-2 py-0.5 rounded border border-red-100' : 'text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100'">
                    {{ g.format }}
                  </span>
                </td>
                <td class="py-3 px-4 text-slate-500 font-medium">{{ g.generatedAt }}</td>
                <td class="py-3 px-4 text-right">
                  <div class="flex items-center justify-end gap-1.5">
                    <button
                      @click="viewGeneratedReportArchive(g)"
                      class="px-2.5 py-1.5 bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200/80 rounded-lg font-bold text-[11px] transition-all cursor-pointer inline-flex items-center gap-1 shadow-sm"
                      title="View Report Details"
                    >
                      <Eye class="w-3.5 h-3.5" />
                    ...
                    <button
                      @click="downloadGeneratedReportArchive(g)"
                    ...
                    <button
                      @click="printGeneratedReportArchive(g)"
                      class="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200/80 rounded-lg font-bold text-[11px] transition-all cursor-pointer inline-flex items-center gap-1 shadow-sm"
                      title="Print Report"
                    >
                      <Printer class="w-3.5 h-3.5" />
...
// ---------- Generation Engine ----------
const generateOnScreenReport = async (overrideScheduledInfo?: any) => {
...
const scheduledReportsList = ref<any[]>([]);
const generatedReportsList = ref<any[]>([]);
const activeSubTab = ref<'scheduled' | 'history'>('scheduled');
...
const displayScheduledReports = computed(() => {
  ...
  if (frequencyFilter.value !== 'all') {
    list = list.filter((s: any) => s.frequency === frequencyFilter.value);
  }
  return list;
});
...
const displayGeneratedReports = computed(() => {
  ...
  if (frequencyFilter.value !== 'all') {
    list = list.filter((g: any) => g.frequency === frequencyFilter.value);
  }
  return list;
});
...
const downloadScheduledReport = async (s: any) => {
  try {
...
const printGeneratedReportArchive = (g: any) => {
  if (g.library_id) selectedLibraryId.value = g.library_id;
  if (g.report_type) filters.category = g.report_type;
  generateOnScreenReport(g);
  setTimeout(() => {
