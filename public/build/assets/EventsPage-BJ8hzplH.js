import { d as defineComponent, r as ref, x as useAuth, c as computed, o as onMounted, a as createElementBlock, b as createBaseVNode, l as createCommentVNode, f as createVNode, n as normalizeClass, u as unref, t as toDisplayString, F as Fragment, j as renderList, h as createBlock, i as createTextVNode, q as normalizeStyle, g as resolveComponent, k as openBlock } from "./main-DKJzpCzk.js";
import { E as E$1 } from "./jspdf.es.min-D5317V0e.js";
...
const VALID_VALUES = new Set(Object.values(MIX_BLEND_MODE));
const mixBlendMode = {
  name: "mix-blend-mode",
  initialValue: "normal",
  type: 2,
  prefix: false,
  parse: (_context, token) => {
    return VALID_VALUES.has(token) ? token : MIX_BLEND_MODE.NORMAL;
  }
};
  ...
  constructor(context, declaration) {
    ...
    if (declaration.display === "none") {
      ...
      for (const [key, descriptor] of standardProps) {
        if (key !== "display") {
          this[key] = parse(context, descriptor, void 0);
        }
      }
...
CSSParsedDeclaration.standardProps = [
  ...
  ["transform", transform$1, "transform"],
    ...
    case 3:
      switch (descriptor.format) {
        case "angle":
          ...
          break;
        case "color":
          result = color$1.parse(context, parser.parseComponentValue());
          break;
        case "image":
  ...
  for (let childNode = node.firstChild, nextNode; childNode; childNode = nextNode) {
    ...
    if (isTextNode(childNode) && childNode.data.length > 0) {
      ...
      if (isSlotElement(childNode) && childNode.assignedNodes) {
        childNode.assignedNodes().forEach((assignedNode) => parseNodeTree(context, assignedNode, parent, root));
      } else {
...
const getTextStrokeLineJoin = () => {
  const currentWindow = typeof window !== "undefined" ? window : void 0;
...
class TextRenderer {
  ...
  renderFillText(text, letterSpacing2, baseline, writingMode2) {
    if (this.canRenderWholeText(letterSpacing2, writingMode2)) {
  ...
  async renderTextNode(text, styles, containerBounds) {
    ...
    text.textBounds.forEach((text2) => {
      paintOrder2.forEach((paintOrderLayer) => {
        switch (paintOrderLayer) {
          case 0: {
            this.renderTextFillWithShadows(text2, styles);
          ...
          case 1: {
            this.renderTextStrokeWithStyle(text2, styles);
            break;
          }
        }
      });
...
const calculateObjectFitRendering = (intrinsicWidth, intrinsicHeight, box, objectFit2) => {
  let sx = 0;
  let sy = 0;
...
class CanvasRenderer {
  ...
  renderReplacedElement(container, curves, image2) {
    ...
    if (image2 && intrinsicWidth > 0 && intrinsicHeight > 0) {
      ...
      this.ctx.save();
      this.ctx.clip();
      const { sx, sy, sw, sh, dx, dy, dw, dh } = calculateObjectFitRendering(intrinsicWidth, intrinsicHeight, box, container.styles.objectFit);
...
const _hoisted_6 = { class: "aspect-video relative overflow-hidden bg-slate-50 rounded-t-2xl border-b border-slate-100 flex items-center justify-center" };
const _hoisted_7 = ["src", "alt"];
const _hoisted_8 = {
  key: 1,
  class: "w-full h-full flex flex-col items-center justify-center bg-slate-50 text-slate-400"
};
const _hoisted_9 = { class: "absolute top-3 right-3" };
const _hoisted_10 = { class: "p-5 flex-1 flex flex-col" };
const _hoisted_11 = { class: "flex justify-between items-start mb-3" };
const _hoisted_12 = { class: "inline-block text-[9px] font-extrabold text-blue-600 bg-blue-50/50 px-2 py-0.5 rounded uppercase tracking-wider" };
const _hoisted_13 = { class: "flex items-center text-[10px] text-slate-400 font-bold uppercase tracking-wide" };
const _hoisted_14 = ["title"];
const _hoisted_15 = { class: "text-xs text-slate-500 mb-4 line-clamp-2 leading-relaxed" };
const _hoisted_16 = { class: "space-y-2 mb-4" };
const _hoisted_17 = { class: "flex items-center text-xs text-slate-600 font-semibold" };
const _hoisted_18 = {
  key: 0,
  class: "flex items-center text-xs text-slate-600 font-semibold"
};
const _hoisted_19 = { class: "mt-auto pt-4 border-t border-slate-100 flex flex-col space-y-2" };
const _hoisted_20 = { class: "flex items-center justify-between text-[10px] text-slate-400 font-extrabold uppercase tracking-wide" };
const _hoisted_21 = { class: "w-full bg-slate-100 rounded-full h-1.5 overflow-hidden" };
const _hoisted_22 = { class: "pt-4 mt-3 flex items-center justify-between gap-3" };
const _hoisted_23 = {
  key: 0,
...
const _hoisted_24 = {
  disabled: "",
  class: "w-full py-2 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-xl text-xs font-extrabold flex items-center justify-center space-x-1 cursor-default leading-none"
};
...
const _hoisted_28 = {
  key: 2,
  class: "text-center py-20 bg-slate-100 rounded-2xl border border-dashed border-slate-200/80 p-8"
};
const _hoisted_29 = { class: "w-16 h-16 bg-slate-100 text-slate-400 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-200 shadow-inner" };
const _hoisted_30 = {
  key: 3,
  class: "fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md overflow-y-auto animate-in fade-in duration-300"
};
const _hoisted_31 = { class: "max-w-md w-full bg-slate-100 rounded-[32px] border border-slate-200 shadow-2xl overflow-hidden relative flex flex-col my-8" };
const _hoisted_32 = { class: "p-5 flex items-center justify-between border-b border-slate-200/80 bg-slate-50" };
const _hoisted_33 = { class: "p-6 flex justify-center bg-slate-100/50" };
const _hoisted_34 = { class: "p-5 bg-gradient-to-r from-blue-600 to-blue-700 border-b border-blue-700/20 flex items-center space-x-3 text-white" };
const _hoisted_35 = { class: "w-10 h-10 rounded-xl bg-white/10 border border-white/20 flex items-center justify-center overflow-hidden flex-shrink-0" };
const _hoisted_36 = ["src"];
const _hoisted_37 = { class: "p-5 space-y-4" };
const _hoisted_38 = { class: "space-y-1" };
const _hoisted_39 = { class: "inline-block text-[9px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-150 uppercase tracking-widest" };
const _hoisted_40 = { class: "text-lg font-extrabold leading-tight text-slate-900 tracking-tight line-clamp-2" };
const _hoisted_41 = { class: "grid grid-cols-2 gap-4 bg-slate-50 border border-slate-100 rounded-xl p-3.5" };
const _hoisted_42 = { class: "text-[11px] font-extrabold text-slate-800 mt-0.5" };
const _hoisted_43 = { class: "text-[11px] font-extrabold text-slate-800 mt-0.5" };
const _hoisted_44 = { class: "text-[11px] font-extrabold text-slate-800 mt-0.5 truncate" };
const _hoisted_45 = { class: "p-5 flex items-center justify-between space-x-4 bg-slate-50/50" };
const _hoisted_46 = { class: "min-w-0 flex-1 space-y-4" };
const _hoisted_47 = { class: "flex items-center space-x-3" };
const _hoisted_48 = { class: "w-12 h-12 rounded-xl overflow-hidden border border-slate-200 shadow-inner bg-slate-100 flex-shrink-0" };
const _hoisted_49 = ["src"];
const _hoisted_50 = { class: "min-w-0" };
const _hoisted_51 = { class: "text-xs font-black truncate text-slate-900 leading-snug" };
const _hoisted_52 = { class: "text-[11px] font-mono font-bold text-slate-700 mt-0.5" };
const _hoisted_53 = { class: "flex-shrink-0 flex flex-col items-center space-y-1" };
const _hoisted_54 = { class: "p-1.5 bg-white rounded-xl shadow-md border border-slate-200" };
const _hoisted_55 = ["src"];
const _hoisted_56 = { class: "text-[7px] font-mono font-bold text-slate-400 tracking-wider" };
const _hoisted_57 = { class: "p-5 border-t border-slate-200 bg-slate-50 grid grid-cols-2 gap-3" };
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const getButtonText = (event) => {
      ...
      if (event.capacity !== void 0 && (event.registered_count || 0) >= event.capacity) {
    ...
    const studentPhotoUrl = computed(() => {
      ...
      return rawPhoto ? rawPhoto.startsWith("http") ? rawPhoto : `/storage/${rawPhoto}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(user.value.name)}`;
    });
    ...
    const downloadPassImage = async () => {
      ...
      try {
        const opt = {
          ...
          useCORS: true,
          // Allow cross-origin images (like QR server & avatar api)
    ...
    return (_ctx, _cache) => {
      const _component_Printer = resolveComponent("Printer");
      return openBlock(), createElementBlock("div", _hoisted_1, [
        createBaseVNode("div", _hoisted_2, [
          createBaseVNode("button", {
            ...
            createBaseVNode("span", null, toDisplayString(loading.value ? "Refreshing..." : "Refresh Events"), 1)
          ], 8, _hoisted_3)
        ]),
        loading.value ? (openBlock(), createElementBlock("div", _hoisted_4, [..._cache[0] || (_cache[0] = [
          createBaseVNode("div", { class: "animate-spin rounded-full h-10 w-10 border-2 border-blue-600 border-t-transparent" }, null, -1),
          createBaseVNode("p", { class: "text-xs font-semibold text-slate-400 uppercase tracking-wider animate-pulse" }, "Loading events...", -1)
            ...
            return openBlock(), createElementBlock("div", {
              ...
              createBaseVNode("div", _hoisted_6, [
                event.image ? (openBlock(), createElementBlock("img", {
                  ...
                  createVNode(unref(Calendar), { class: "w-10 h-10 stroke-1" })
                ])),
                createBaseVNode("div", _hoisted_9, [
                  createBaseVNode("span", {
                    class: normalizeClass([
                      "px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider shadow-sm border",
                      event.is_paid ? "bg-gradient-to-r from-amber-500 to-orange-500 text-white border-amber-400/20" : "bg-gradient-to-r from-emerald-500 to-teal-500 text-white border-emerald-400/20"
                    ])
                  }, toDisplayString(event.is_paid ? event.price ? `Rs. ${event.price}` : "Paid" : "Free"), 3)
                ])
              ]),
              createBaseVNode("div", _hoisted_10, [
                createBaseVNode("div", _hoisted_11, [
                  createBaseVNode("span", _hoisted_12, toDisplayString(event.type.replace("_", " ")), 1),
                  createBaseVNode("div", _hoisted_13, [
                    createVNode(unref(Calendar), { class: "w-3.5 h-3.5 mr-1" }),
                    createTextVNode(" " + toDisplayString(formatDate(event.date)), 1)
                  ])
                ]),
                createBaseVNode("h3", {
                  class: "font-bold text-slate-800 text-base mb-1.5 leading-snug group-hover:text-blue-600 transition-colors line-clamp-1",
                  title: event.title
                }, toDisplayString(event.title), 9, _hoisted_14),
                createBaseVNode("p", _hoisted_15, toDisplayString(event.description), 1),
                createBaseVNode("div", _hoisted_16, [
                  ...
                  event.venue ? (openBlock(), createElementBlock("div", _hoisted_18, [
                    createVNode(unref(MapPin), { class: "w-4 h-4 mr-2 text-slate-400" }),
                    createTextVNode(" " + toDisplayString(event.venue), 1)
                  ])) : createCommentVNode("", true)
                ]),
                createBaseVNode("div", _hoisted_19, [
                  createBaseVNode("div", _hoisted_20, [
                    _cache[1] || (_cache[1] = createBaseVNode("span", null, "Attendees", -1)),
                    createBaseVNode("span", null, toDisplayString(event.registered_count || 0) + " / " + toDisplayString(event.capacity || "Unlimited"), 1)
                  ]),
                  createBaseVNode("div", _hoisted_21, [
                    createBaseVNode("div", {
                      class: "bg-gradient-to-r from-blue-650 to-cyan-500 h-full rounded-full transition-all duration-700",
                      style: normalizeStyle({ width: event.capacity ? `${Math.min(100, Math.round((event.registered_count || 0) / event.capacity * 100))}%` : "0%" })
                    }, null, 4)
                  ])
                ]),
                createBaseVNode("div", _hoisted_22, [
                  event.is_registered ? (openBlock(), createElementBlock("div", _hoisted_23, [
                    createBaseVNode("button", _hoisted_24, [
                      createVNode(unref(CircleCheckBig), { class: "w-4 h-4" }),
                      _cache[2] || (_cache[2] = createBaseVNode("span", null, "Already Registered", -1))
                    ]),
                    createBaseVNode("button", {
                      onClick: ($event) => viewPass(event),
                      class: "w-full py-2 bg-white border border-blue-200 text-blue-600 hover:bg-blue-50/50 rounded-xl text-xs font-bold transition-all flex items-center justify-center space-x-2 active:scale-98 shadow-sm leading-none"
                    }, [
                      createVNode(_component_Printer, { class: "w-4 h-4 text-blue-500" }),
                      _cache[3] || (_cache[3] = createBaseVNode("span", null, "View Pass", -1))
                    ], 8, _hoisted_25)
                  ])) : (openBlock(), createElementBlock("button", {
                    key: 1,
                    onClick: ($event) => joinEvent(event),
                    disabled: joiningId.value === event.id || isRegistrationClosed(event) || event.capacity !== void 0 && (event.registered_count || 0) >= event.capacity,
                    class: "w-full py-2.5 bg-blue-600 hover:from-blue-700 hover:to-cyan-600 disabled:from-slate-100 disabled:to-slate-100 disabled:text-slate-400 disabled:shadow-none disabled:border-slate-200/50 disabled:cursor-not-allowed text-white text-xs font-extrabold rounded-xl shadow-md shadow-blue-500/10 active:scale-98 transition-all flex items-center justify-center space-x-2 leading-none"
                  }, [
                    joiningId.value === event.id ? (openBlock(), createElementBlock("span", _hoisted_27)) : createCommentVNode("", true),
                    createBaseVNode("span", null, toDisplayString(getButtonText(event)), 1)
                  ], 8, _hoisted_26))
                ])
              ])
            ]);
          ...
          createBaseVNode("div", _hoisted_29, [
            createVNode(unref(Calendar), { class: "w-7 h-7" })
          ]),
          _cache[4] || (_cache[4] = createBaseVNode("h3", { class: "text-base font-bold text-slate-800 mb-1" }, "No upcoming events", -1)),
          _cache[5] || (_cache[5] = createBaseVNode("p", { class: "text-xs text-slate-400 max-w-sm mx-auto leading-relaxed" }, "Check back later for newly scheduled workshops and seminars.", -1))
        ])),
        isPassModalOpen.value ? (openBlock(), createElementBlock("div", _hoisted_30, [
          createBaseVNode("div", _hoisted_31, [
            createBaseVNode("div", _hoisted_32, [
              _cache[6] || (_cache[6] = createBaseVNode("span", { class: "text-xs font-black uppercase tracking-widest text-slate-500" }, "Digital Event Pass", -1)),
              createBaseVNode("button", {
                onClick: closePassModal,
                class: "p-2 hover:bg-slate-200 text-slate-500 hover:text-slate-800 rounded-full transition-colors"
              }, [
            ...
            createBaseVNode("div", _hoisted_33, [
              createBaseVNode("div", {
                ref_key: "passRef",
                ref: passRef,
                class: "w-full max-w-sm bg-white rounded-[24px] border border-slate-200/80 shadow-lg overflow-hidden relative flex flex-col font-outfit text-slate-800",
                style: { "width": "360px" }
              }, [
                createBaseVNode("div", _hoisted_34, [
                  createBaseVNode("div", _hoisted_35, [
                    selectedPassEvent.value?.library?.photo ? (openBlock(), createElementBlock("img", {
                      key: 0,
                      src: selectedPassEvent.value.library.photo,
                      class: "w-full h-full object-cover"
                    }, null, 8, _hoisted_36)) : (openBlock(), createBlock(unref(BookOpen), {
                      key: 1,
                      class: "w-5 h-5 text-white"
                    }))
                  ]),
                  _cache[7] || (_cache[7] = createBaseVNode("div", { class: "min-w-0 flex-1" }, [
                    createBaseVNode("h4", { class: "text-sm font-black uppercase tracking-[0.15em] text-white" }, "GATE PASS")
                  ], -1))
                ]),
                createBaseVNode("div", _hoisted_37, [
                  createBaseVNode("div", _hoisted_38, [
                    createBaseVNode("span", _hoisted_39, toDisplayString(selectedPassEvent.value?.type?.replace("_", " ") || "Workshop"), 1),
                    createBaseVNode("h3", _hoisted_40, toDisplayString(selectedPassEvent.value?.title), 1)
                  ]),
                  createBaseVNode("div", _hoisted_41, [
                    createBaseVNode("div", null, [
                      _cache[8] || (_cache[8] = createBaseVNode("span", { class: "text-[8px] font-bold text-slate-400 uppercase tracking-widest" }, "Date", -1)),
                      createBaseVNode("p", _hoisted_42, toDisplayString(selectedPassEvent.value ? formatDate(selectedPassEvent.value.date) : ""), 1)
                    ]),
                    createBaseVNode("div", null, [
                      _cache[9] || (_cache[9] = createBaseVNode("span", { class: "text-[8px] font-bold text-slate-400 uppercase tracking-widest" }, "Time", -1)),
                      createBaseVNode("p", _hoisted_43, toDisplayString(selectedPassEvent.value ? formatTime(selectedPassEvent.value.start_time) : ""), 1)
                    ]),
                    createBaseVNode("div", null, [
                      _cache[10] || (_cache[10] = createBaseVNode("span", { class: "text-[8px] font-bold text-slate-400 uppercase tracking-widest" }, "Venue", -1)),
                      createBaseVNode("p", _hoisted_44, toDisplayString(selectedPassEvent.value?.venue || "Main Hall"), 1)
                    ]),
                    _cache[11] || (_cache[11] = createBaseVNode("div", null, [
                      createBaseVNode("span", { class: "text-[8px] font-bold text-slate-400 uppercase tracking-widest" }, "Seat"),
                      createBaseVNode("p", { class: "text-[11px] font-extrabold text-blue-600 mt-0.5" }, "General Admission")
                    ], -1))
                  ])
                ]),
                _cache[15] || (_cache[15] = createBaseVNode("div", { class: "relative flex items-center justify-between px-3" }, [
                  createBaseVNode("div", { class: "w-6 h-6 rounded-full bg-slate-100 border border-slate-200/80 -ml-6 z-10" }),
                  createBaseVNode("div", { class: "w-full border-t-2 border-dashed border-slate-200" }),
                  createBaseVNode("div", { class: "w-6 h-6 rounded-full bg-slate-100 border border-slate-200/80 -mr-6 z-10" })
                ], -1)),
                createBaseVNode("div", _hoisted_45, [
                  createBaseVNode("div", _hoisted_46, [
                    createBaseVNode("div", _hoisted_47, [
                      createBaseVNode("div", _hoisted_48, [
                        createBaseVNode("img", {
                          src: studentPhotoUrl.value,
                          class: "w-full h-full object-cover"
                        }, null, 8, _hoisted_49)
                      ]),
                      createBaseVNode("div", _hoisted_50, [
                        _cache[12] || (_cache[12] = createBaseVNode("span", { class: "text-[8px] font-bold text-slate-400 uppercase tracking-widest" }, "Attendee", -1)),
                        createBaseVNode("h5", _hoisted_51, toDisplayString(unref(user)?.name), 1),
                        _cache[13] || (_cache[13] = createBaseVNode("p", { class: "text-[9px] text-blue-600 font-extrabold uppercase mt-0.5" }, "Verified", -1))
                      ])
                    ]),
                    createBaseVNode("div", null, [
                      _cache[14] || (_cache[14] = createBaseVNode("span", { class: "text-[8px] font-bold text-slate-400 uppercase tracking-widest" }, "Student CRN", -1)),
                      createBaseVNode("p", _hoisted_52, toDisplayString(unref(user)?.crn || "N/A"), 1)
                    ])
                  ]),
                  createBaseVNode("div", _hoisted_53, [
                    ...
                    createBaseVNode("span", _hoisted_56, "PASS-" + toDisplayString(selectedPassEvent.value?.id) + "-" + toDisplayString(unref(user)?.id), 1)
                  ])
                ])
              ], 512)
            ]),
            createBaseVNode("div", _hoisted_57, [
              createBaseVNode("button", {
                ...
                class: "py-2.5 px-3 bg-white hover:bg-slate-100 text-slate-800 rounded-xl text-xs font-bold transition-all border border-slate-200 flex items-center justify-center space-x-1.5 active:scale-95 disabled:opacity-50"
              }, [
                createVNode(unref(Download), { class: "w-3.5 h-3.5" }),
                _cache[16] || (_cache[16] = createBaseVNode("span", null, "Image", -1))
              ], 8, _hoisted_58),
              createBaseVNode("button", {
                ...
                createVNode(unref(FileText), { class: "w-3.5 h-3.5 text-blue-600" }),
                _cache[17] || (_cache[17] = createBaseVNode("span", null, "PDF", -1))
              ], 8, _hoisted_59)
            ])
          ])
        ])) : createCommentVNode("", true)
...
const EventsPage = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-9ad80482"]]);
export {
